...
最终全局保护。默认值为 `TOOLBOX_PRODUCT_CUTOUT_CONCURRENCY=2`，上限也是 2。
Workbench 传入的 `productFolderKey` 是不可逆摘要，不含 UNC 路径。

上限由显存决定，不是拍脑袋定的：抠图在 2080 Ti 上单进程占约 14.8 GiB / 22 GiB，
两个并发峰值 22.1 GiB（只剩不到 2% 余量），三个直接 CUDA 崩溃。而且第二个进程
只带来约 20% 提速——一个进程就已经把 GPU 喂饱，多开只是排队。Workbench 那边
仍可提交更多，多出来的在网关排队，正好让显卡在两张图之间不空转。
