...
- **主图**：对每张符合“商品整体图”条件的原始拍摄图，生成一张 800×800 PNG 成品，
  发布到 `主图/`。背景归一为白色，**保留**实测投影。
...
一个 3–7 色的商品因此增加 3–7 次独占运行（实测每次约 12–25 秒）。代表图同时也是
主图分支的输入，所以同一帧会被归一两次（主图 `SHADOW_GAIN=1`、SKU `=64`）；两个
分支按设计各自独立成败，没有共享中间产物，这是已知的重复开销。
