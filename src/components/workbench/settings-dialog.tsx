...
const GROUP_META: Record<string, { label: string; sub: string }> = {
  ...
  monoImage: { label: "生图服务（Mono）", sub: "AI 生图服务地址；主图可切换 mono-image 或 comfyui。" },
  videoGeneration: {
    label: "视频生成服务（Mono）",
    sub: "本地 ComfyUI（服务机）和云端 DashScope（百炼）可以同时启用，凭证配齐的都会出现在模型列表里；Provider 只用于「自动」选择时的优先级。",
  },
  videoStorage: {
    label: "大视频云存储（TOS）",
...
const FIELD_LABEL: Record<string, string> = {
  ...
  MONO_IMAGE_MODEL: "模型",
  PRODUCT_MAIN_IMAGE_PROVIDER: "主图 Provider（mono-image / comfyui）",
  VIDEO_GENERATION_PROVIDER: "「自动」优先 Provider（comfyui / dashscope-wan，可选）",
  VIDEO_GENERATION_BASE_URL: "Base URL（DashScope/百炼）",
  VIDEO_GENERATION_API_KEY: "API Key（DashScope/百炼）",
  VIDEO_GENERATION_T2V_MODEL: "文生模型（默认 wan2.7-t2v，另有 HappyHorse 1.1 可选）",
  VIDEO_GENERATION_I2V_MODEL: "图生模型（默认 wan2.7-i2v，另有 HappyHorse 1.1 可选）",
  TOS_AK: "AccessKeyId",
