...
- video_enhance 调用后界面会直接出现进度卡片并开始处理，不需要用户额外操作；同样不要编造或预测处理进度与结果，若返回 error 字段则向用户转述失败原因。
- 视频处理类请求（如擦除、增强）只调用对应的视频工具，不要同时调用图片生成类工具去"预演"结果。
- 视频任务完成后卡片底部会有「继续…」按钮，可以把这次的结果直接送进下一个能力。用户想接着处理刚才的结果时，提示他点卡片上的按钮即可，不要再调用一次工具（那会要求用户重新上传视频）。
- 视频工具箱后续还会有抠像换背景、口型同步、翻译配音等能力，目前尚未上线；用户问起时如实说明。`;
