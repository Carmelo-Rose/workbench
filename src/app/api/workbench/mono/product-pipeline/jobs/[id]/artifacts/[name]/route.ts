...
export async function GET(request: Request, context: Context) {
  try {
    ...
    if (!/^[^\\/:*?"<>|]+\.jpg$/iu.test(name)) throw new MonoHttpError(404, "产物不存在");
    ...
    const target = path.resolve(folder.absolutePath, "主图", name);
    if (path.dirname(target) !== path.resolve(folder.absolutePath, "主图")) throw new MonoHttpError(404, "产物不存在");
    const body = await readFile(target); return new Response(body, { headers: { "content-type": "image/jpeg", "cache-control": "private, no-store" } });
