...
export async function POST(request: Request) {
  try { const actor = actorFromWorkbenchRequest(request, "image.product-set.use"); const input = await parseMonoJson(request, productPipelineInputSchema); return Response.json({ job: lightenMonoJob(createProductPipelineJob(actor, input)) }, { status: 201 }); }
